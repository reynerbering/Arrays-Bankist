...
const foodChecker = function (curFood, recFood) {
  if (curFood > recFood * 0.9 && curFood < recFood * 1.1) {
    return 'Eating okay!';
  } else if (curFood > recFood) {
...
console.log(foodChecker(sarahDog.curFood, sarahDog.recFood));

// 3
const ownersTooMuch = dogs
  .filter(dog => foodChecker(dog.curFood, dog.recFood) === 'Eating too much')
  .flatMap(dog => dog.owners);

// owners of dogs who eat too little
const ownersTooLittle = dogs
  .filter(dog => foodChecker(dog.curFood, dog.recFood) === 'Eating too little')
  .flatMap(dog => dog.owners);
...
// 4
// const littleArray = ownersTooLittle.join(' and ', '');
...
const groupedDogs = Object.groupBy(dogs, dog => {
  if (foodChecker(dog.curFood, dog.recFood) === 'Eating too much') {
    return 'too-much';
  } else if (foodChecker(dog.curFood, dog.recFood) === 'Eating too little') {
    return 'too-little';
  } else {
    return 'exact';
  }
